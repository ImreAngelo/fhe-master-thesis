...
#define CENTER(integer, modulus) ((integer) % (modulus))
...
inline void RunTest(const std::vector<int64_t>& value, const int64_t log_b = 30) {
    DEBUG_PRINT("Testing RGSW with value = " << value << " (size = " << value.size() << ")");
    ...
    Plaintext pt = cc->MakeCoefPackedPlaintext(std::vector<int64_t>(value.size(), 1));
    auto rlwe_ct = cc->Encrypt(keyPair.publicKey, pt);
    auto rgsw_ct = cc->Encrypt_Textbook(keyPair.publicKey, cc->MakeCoefPackedPlaintext(value), log_b, ell);
    ...
    // Test External Product
    {
        ...
        DEBUG_PRINT("Decrypted result 1: " << result_slot);
