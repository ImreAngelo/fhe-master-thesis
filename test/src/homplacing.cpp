...
TEST(HomPlacing, x2) { TestHomPlacing({1, 0},           4); }
 ...
 * Should place value in slot 2^index
 */
inline void TestHomPlacing2(const std::vector<int64_t>& index, const int64_t& value) {
    CCParams<CryptoContextRGSWBGV> params;
    params.SetMultiplicativeDepth(2);
    params.SetPlaintextModulus(65537);
    params.SetRingDim(16384);
...
// Basic test
TEST(HomPlacingBetterParams, b0) { TestHomPlacing2({0}, 4); }
TEST(HomPlacingBetterParams, b1) { TestHomPlacing2({1}, 4); }

TEST(HomPlacingBetterParams, x2) { TestHomPlacing2({1, 0},        4); }
TEST(HomPlacingBetterParams, x3) { TestHomPlacing2({0, 1, 1},     4); }
TEST(HomPlacingBetterParams, x4) { TestHomPlacing2({1, 0, 1, 1},  4); }