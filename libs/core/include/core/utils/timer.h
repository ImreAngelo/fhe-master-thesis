 ...
 */
struct Timer {
    explicit Timer(std::string&& label) : m_label(std::move(label)), m_start(Clock::now()) {}

    ~Timer() {
        double ms = std::chrono::duration_cast<Ms>(Clock::now() - m_start).count();
        std::cout << "  [timing] " << m_label << ": " << std::fixed << std::setprecision(2) << ms << " ms\n";
    ...
    double elapsed_ms() const { return std::chrono::duration_cast<Ms>(Clock::now() - m_start).count(); }
